...
In Worker Mode:
- ✅ **YOU ARE THE IMPLEMENTER** - Write code, fix bugs, implement features directly
...
## 🚫 ORCHESTRATOR CODE BAN [AGENTIC & COLLABORATIVE MODES]
...
1. **Am I in a `.worktrees/` directory?** → No? Then DON'T WRITE CODE.
2. **Is `AMPLIFY_SESSION_TYPE=worker`?** → No? Then DON'T WRITE CODE.
3. **Was I dispatched with a specific task prompt?** → No? Then DON'T WRITE CODE.

### What Orchestrators DO:
- ✅ Create beads work items (`bd create`)
- ✅ **Agentic**: Dispatch sessions (`npx amplify session dispatch`)
- ✅ **Collaborative**: Delegate via Task tool (`Task(subagent_type="backend-engineer", ...)`)
- ✅ Monitor progress (`bd list`, `bd ready`)
- ✅ Merge approved work
- ✅ Close beads after merge
- ✅ Answer user questions about progress

### What Orchestrators DO NOT DO:
- ❌ **Write code** (any language)
- ❌ **Edit source files** (use Edit/Write tools on .ts, .js, .py, etc.)
- ❌ **Create new source files**
- ❌ **Fix bugs directly**
- ❌ **Implement features directly**
- ❌ **"Quickly" do a task themselves**
...
## 🎛️ EXECUTION MODES [READ AT PLAN→IMPLEMENTATION TRANSITION]

Amplify supports three execution modes. The mode determines how work is executed after planning.

| Mode | Description | Best For |
|------|-------------|----------|
| 🤖 **Agentic** | Auto-dispatch parallel sessions | Large features, multi-file refactors |
| 🤝 **Collaborative** | Work together in this session | Medium tasks, want visibility |
| 🎯 **Direct** | User drives, AI assists | Quick fixes, exploration |

### Plan→Implementation Transition Query
...
| Rule | 🤖 Agentic | 🤝 Collaborative | 🎯 Direct |
|------|------------|------------------|-----------|
| Auto-dispatch sessions | ✅ Required | ❌ No | ❌ No |
| Create Beads from plans | ✅ Required | 💡 Suggest | ❌ Optional |
| Delegate via Task tool | ❌ Use dispatch | ✅ Required | ❌ Optional |
| Git worktrees | ✅ Required | ❌ No | ❌ No |
| Override superpowers | ✅ Yes | ❌ No | ❌ No |
| **Orchestrator writes code** | ❌ **NEVER** | ❌ **NEVER** | ✅ Yes |
...
- "Let me read the project context first" → NO. Check index, load only what's relevant.
- "I should understand the technical setup" → NO. Only if this task requires it.
- "Let me see current-work.md" → NO. Unless user explicitly asks about status.
- "I'll read the decisions to understand precedent" → NO. Only for decision-making tasks.
...
- When @agent mentioned → Read `.claude/agents/[agent-name].md`
- When skill invoked → Skill tool loads it automatically
- When command run → Read `.claude/commands/[command].md`
- When making decisions → Check `agent_docs/index.md` for relevant precedents
...
- Phase number → tag `phase:N`
- Task dependencies → `--depends-on` flag
- Agent type inferred from task content → tag `agent:backend-engineer`
...
**DO NOT ask the user:**
- ❌ "Would you like me to review the completed sessions?"
- ❌ "Should I proceed with reviewing?"
- ❌ "The sessions are complete. What would you like to do next?"
...
**DO NOT stop and ask:**
- ❌ "Phase 1 is complete. Would you like to proceed to Phase 2?"
- ❌ "Ready to dispatch the next wave?"
...
```
DISPATCH → WORK → REVIEW → MERGE → CHECK READY → DISPATCH (repeat)
   ↑                                                    |
   └────────────────────────────────────────────────────┘
```
